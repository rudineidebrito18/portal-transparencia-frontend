...
import { conselhoSaudeService } from '@/modules/admin/institucional/conselhoSaude.service'
import { ConselhoMunicipal, ConselhoMunicipalRequest, TipoConselho } from '@/modules/conselho-saude/types'
...
export default function ConselhoSaudeAdminPage() {
  ...
  async function confirmarExclusao() {
    ...
    setExcluindo(true)
    try {
      await conselhoSaudeService.excluir(idParaExcluir)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const dados: ConselhoMunicipalRequest = {
    ...
    try {
      if (form.id) {
        await conselhoSaudeService.atualizar(form.id, dados)
      } else {
        await conselhoSaudeService.criar(dados)
      }
      setForm(null)
      carregar()
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-admin-text">Conselho Municipal de Saúde</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {form.id && (
              <p className="text-xs text-admin-text-faint">
                ...
                <Link href={`/admin/institucional/conselho-saude/${form.id}`} className="text-admin-accent hover:underline">
                  ver detalhes
                </Link>
                ).
              </p>
                  ...
                  <tr key={c.id} className="border-t border-admin-border hover:bg-admin-surface-2/60 transition-colors">
                    ...
                    <td className="p-3.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          href={`/admin/institucional/conselho-saude/${c.id}`}
