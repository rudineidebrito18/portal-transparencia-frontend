...
const FORM_VAZIO: FormState = { id: null, titulo: '', texto: '', data: '', ativo: true, imagemAtual: null }
...
export default function NoticiasAdminPage() {
  ...
  function abrirEdicao(item: ConteudoInstitucional) {
    ...
    setForm({
      ...
      data: item.data,
      ativo: item.ativo,
      imagemAtual: item.imagemUrl ?? null
    })
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (form.id) {
        await noticiaAdminService.atualizar(form.id, dados, imagem)
      } else {
        await noticiaAdminService.criar(dados, imagem)
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">
                Imagem {form.id && '(opcional — mantém a atual se vazio)'}
              </label>
              {form.imagemAtual && !imagem && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={form.imagemAtual} alt="Imagem atual" className="h-24 rounded-lg mb-2 object-cover" />
              )}
              <input
                type="file"
              ...
              />
              {imagem && <p className="text-xs text-text-secondary/70 mt-1">Selecionada: {imagem.name}</p>}
            </div>
              ...
              {data.map(item => (
                <tr key={item.id} className="border-t border-border/20">
                  <td className="p-3">
                    {item.imagemUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={item.imagemUrl} alt="" className="h-10 w-10 rounded-lg object-cover" />
                    ) : (
