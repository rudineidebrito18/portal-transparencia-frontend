...
export default function TabelaValoresAdminPage() {
  ...
  async function confirmarExclusao() {
    ...
    setExcluindo(true)
    try {
      await tabelaValoresService.excluir(idParaExcluir)
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-admin-text">Tabela de Valores de Diária</h1>
              ...
              <tbody>
                {data.map(item => (
                  <tr key={item.id} className="border-t border-admin-border hover:bg-admin-surface-2/60 transition-colors">
                    <td className="p-3.5 text-admin-text">{item.descricao}</td>
                    <td className="p-3.5">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-admin-surface-3 text-admin-text-muted">
