...
// 'categoria' é a aba ativa dentro de /diario-oficial (Edições, Legislação, etc. — ver
// DiarioOficialView) — mora na mesma URL, mas não é filtro de busca das edições.
...
// useDocumentoGenericoUrlState (shared/hooks/), pro shape de FiltroEdicaoDiario (inclui termo,
// a busca por palavra-chave). usePageableResource em si não foi tocado.
export function useEdicaoDiarioUrlState(ordenacaoPadrao = 'dataPublicacao,desc') {
  ...
  const filtros = {} as FiltroEdicaoDiario
  ...
  function setFiltros(novosFiltros: FiltroEdicaoDiario) {
